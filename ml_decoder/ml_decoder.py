...
def add_ml_decoder_head(model, num_classes=-1, num_of_groups=-1, decoder_embedding=768, zsl=0):
    ...
    # Check if the model is PEFT wrapped
    is_peft_model = hasattr(model, 'peft_config')
    base_model = model.base_model.model if is_peft_model else model

    if hasattr(base_model, 'global_pool') and hasattr(base_model, 'fc'):  # resnet50
        base_model.global_pool = nn.Identity()
        del base_model.fc
        base_model.fc = MLDecoder(num_classes=num_classes, initial_num_features=num_features, num_of_groups=num_of_groups,
                             decoder_embedding=decoder_embedding, zsl=zsl)
    elif hasattr(base_model, 'head'):  # tresnet
        if hasattr(base_model, 'global_pool'):
            base_model.global_pool = nn.Identity()
        del base_model.head
        base_model.head = MLDecoder(num_classes=num_classes, initial_num_features=num_features, num_of_groups=num_of_groups,
                               decoder_embedding=decoder_embedding, zsl=zsl)
    else:
    ...
    # return model
    return model if is_peft_model else base_model

def add_ml_decoder_head_v2(model, num_classes=-1, num_of_groups=-1, decoder_embedding=768, zsl=0):
    if num_classes == -1:
        num_classes = model.num_classes
    num_features = model.num_features
...
class MLDecoder(nn.Module):
    def __init__(self, num_classes, num_of_groups=-1, decoder_embedding=768,
        ...
        if not zsl:
            # leanable embedding for non zero-shot learning.
    ...
    def forward(self, x, return_label_features=False):
